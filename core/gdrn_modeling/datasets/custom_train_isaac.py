...
class CUSTOM_TRAIN_ISAAC_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/synth_box_test/train_isaac"),
        )
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.custom.id2obj.items() if obj_name in self.objs]
        ...
        self.scenes = [str(i) for i in range(14)]
    ...
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_info_dict, postfix=f"{scene_id}"):
                ...
                depth_path = osp.join(scene_root, "depth/{:04d}.png".format(int_im_id))
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    mask_file = osp.join(
                        scene_root,
                        "mask/{:04d}_{}.png".format(int_im_id, anno_i),
                    )
                    mask_visib_file = osp.join(
                        scene_root,
                        "mask_visib/{:04d}_{}.png".format(int_im_id, anno_i),
                    )
                    ...
                    xyz_path = osp.join(self.xyz_root, f"{scene_id:04d}/{int_im_id:04d}_{anno_i}-xyz.pkl")
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.custom.obj2id[obj_name]:06d}.ply",
                ),
...
def get_custom_metadata(obj_names, ref_key):
...
custom_model_root = "BOP_DATASETS/synth_box_test/models/"
################################################################################


SPLITS_CUSTOM_TRAIN_ISAAC = dict(
    custom_train_isaac=dict(
        name="custom_train_isaac",
        objs=ref.custom.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/synth_box_test/train_isaac"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/synth_box_test/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/synth_box_test/train_isaac/xyz_crop"),
...
# single obj splits
for obj in ref.custom.objects:
    for split in ["train_isaac"]:
        name = "custom_{}_{}".format(obj, split)
        if split in ["train_isaac"]:
            filter_invalid = True
        elif split in ["test"]:
            filter_invalid = False
        else:
            raise ValueError("{}".format(split))
        if name not in SPLITS_CUSTOM_TRAIN_ISAAC:
            SPLITS_CUSTOM_TRAIN_ISAAC[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_CUSTOM_TRAIN_ISAAC:
        used_cfg = SPLITS_CUSTOM_TRAIN_ISAAC[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, CUSTOM_TRAIN_ISAAC_Dataset(used_cfg))
    ...
    MetadataCatalog.get(name).set(
        ...
        evaluator_type="bop",
        **get_custom_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_CUSTOM_TRAIN_ISAAC.keys())
